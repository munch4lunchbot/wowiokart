# Auditing Azeroth Kart

A deep, self-directed pass on the game. Tool-neutral on purpose: paste it into
any coding agent, or point one at this file. Claude Code users get it as
`/audit` (see `.claude/commands/audit.md`, which is a two-line pointer here so
the two can never say different things).

If you were given a focus, start there; otherwise pick the target yourself.
